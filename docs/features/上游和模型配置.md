...
| `disabled` | boolean | 是否禁用（禁用后不出现在模型选择器中，默认 false） |
...
| `sortOrder` | number | 排序顺序（默认 999，数值越小越靠前） |
| `createdAt` | Date | 创建时间 |
...
| `openai-video` | OpenAI Video API | Sora |

### API 路径说明 {#api-paths}

配置上游时，**Base URL 只需填写域名部分**，系统会根据模型的 API 格式自动拼接完整路径。

假设 Base URL 为 `https://api.example.com`，各 API 格式的完整请求路径如下：

| API 格式 | 请求路径 | 完整 URL 示例 |
|---------|---------|--------------|
| `openai-chat` | `/v1/chat/completions` | `https://api.example.com/v1/chat/completions` |
| `claude` | `/v1/messages` | `https://api.example.com/v1/messages` |
| `gemini` | `/v1beta/models/{model}:generateContent` | `https://api.example.com/v1beta/models/gemini-2.0-flash:generateContent` |
| `dalle` | `/v1/images/generations` | `https://api.example.com/v1/images/generations` |
| `mj-proxy` | `/mj/submit/imagine` | `https://api.example.com/mj/submit/imagine` |
| `video-unified` | `/v1/video/create` | `https://api.example.com/v1/video/create` |
| `openai-video` | `/v1/videos` | `https://api.example.com/v1/videos` |
| `koukoutu` | `/v1/create` | `https://api.example.com/v1/create` |

**注意事项**：
- **不要在 Base URL 中添加 `/v1` 后缀**，系统会自动处理
- 不同模型类型使用不同的路径，确保上游服务支持对应的 API 格式
- 如果上游服务的路径与上表不符，可能需要联系服务商确认兼容性
...
- 按分类筛选（对话/图片/视频/TTS）
- 按厂商筛选（OpenAI/Anthropic/Google 等）
- 拖拽排序（调整模型在选择器中的显示顺序）
- 批量管理（全选/批量删除）
...
- 点击「批量管理」按钮进入批量模式
- 批量模式下可勾选多个模型
- 支持全选/取消全选（仅作用于当前筛选结果）
- 点击「删除选中」移除选中的模型（保存后生效）
- 再次点击「取消批量」退出批量模式
