...
const IMAGE_TIMEOUT_MS = 3 * 60 * 1000 // 3 分钟
...
export async function generateImage(
  ...
  // 阻塞模式：等待任务完成
  const result = await waitForTask(task.id, user.id, IMAGE_TIMEOUT_MS)
  ...
  if (result.status === 'timeout') {
    return {
      content: [{ type: 'text' as const, text: JSON.stringify({
        taskId: result.taskId,
        status: 'timeout',
        message: 'Task did not complete within 3 minutes. Use get_task to check status later.',
      }) }],
    }
  }
